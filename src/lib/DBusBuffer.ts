...
export class DBusBuffer {
    ...
    public readSInt16(): number {
        if (this.#position + 2 > this.#buffer.length) {
            throw new RangeError(`Buffer out of range: position ${this.#position + 2} exceeds length ${this.#buffer.length}`)
        }
    ...
    public readSInt32(): number {
        if (this.#position + 4 > this.#buffer.length) {
            throw new RangeError(`Buffer out of range: position ${this.#position + 4} exceeds length ${this.#buffer.length}`)
        }
    ...
    public readInt32(): number {
        ...
        this.#position += 4
        if (res < 0) {
            throw new RangeError(`Invalid length value read: ${res} at position ${this.#position - 4}`)
        }
        return res
    }
    ...
    public readSInt64(): bigint {
        if (this.#position + 8 > this.#buffer.length) {
            throw new RangeError(`Buffer out of range: position ${this.#position + 8} exceeds length ${this.#buffer.length}`)
        }
    ...
    public readString(len: number): string {
        ...
        if (this.#position + len > this.#buffer.length) {
            throw new RangeError(`Buffer out of range: position ${this.#position + len} exceeds length ${this.#buffer.length}`)
        }
    ...
    public readArray(eleType: DataType, arrayBlobSize: number): any {
        ...
        if (this.#position + arrayBlobSize > this.#buffer.length) {
            throw new RangeError(`Buffer out of range: array size ${arrayBlobSize} at position ${this.#position} exceeds buffer length ${this.#buffer.length}`)
        }
        let arrayResult: any = []
