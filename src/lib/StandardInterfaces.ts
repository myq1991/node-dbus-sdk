...
export function stdDbusIfaces(msg: IDBusMessage, bus: MessageBus) {
    if (
        ...
        const propertiesReply: IDBusMessage = {
            type: messageType.methodReturn,
            serial: bus.serial++,
            replySerial: msg.serial,
            destination: msg.sender
...
function interfaceToXML(iface) {
    ...
    let dumpArgs = function (argsSignature, argsNames, direction) {
        ...
        let args = Signature.parseSignature(argsSignature)
